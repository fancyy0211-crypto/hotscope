...
export const createRealZhihuTopicSource = (fallbackSource: PlatformTopicSourceLike): PlatformTopicSourceLike => ({
  async fetchCurrentTopics(): Promise<HotTopic[]> {
    try {
      ...
      if (sanitized.length === 0) {
        throw new Error('No valid zhihu rows from API');
      }
